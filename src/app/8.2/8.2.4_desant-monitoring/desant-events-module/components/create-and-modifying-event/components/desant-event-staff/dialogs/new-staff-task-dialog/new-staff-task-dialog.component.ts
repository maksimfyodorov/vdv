...
import {
  COLS_WITHOUT_HEIGHT
} from '../../../../../../../../../shared/components/ospo/ospo-coordinates/infrastucture/table-cols';
...
@Component({
  selector: 'app-new-full-desant-task',
  templateUrl: './new-staff-task-dialog.component.html',
  styleUrls: ['./new-staff-task-dialog.component.scss'],
  providers: [DesantEventsService, OlApiService, DialogService, LoaderService],
})
export class NewStaffTaskDialogComponent implements OnInit {
  ...
  public createOrChangeTask(): void {
    this.loader.startLoading(this.newEventService.createNewStaffTask(this.setTaskParams())).subscribe(value => {
  ...
  public changeTask(): void {
    this.loader.startLoading(this.newEventService.changeStaffTask(this.newTaskUUID, this.setTaskParams())).subscribe();
  }
  ...
  public addDesantSubdivision(): void {
    this.dialogService.open(SelectDesantSubdivisionComponent, {
      ...
      if (uuid) {
        this.loader.startLoading(this.newEventService.getCompletingInfo(this.getQueryParamsForEventCompleting(this.newTaskUUID)))
          .subscribe(result => this.completingTableInfo = result.data);
      }
    });
  }
  ...
  public deleteCompleting(uuid: string): void {
    this.dialogService.open(ConfirmationDialogComponent, {
      dismissableMask: true,
      header: `Удалить этап`, data: {
        message: 'Вы действительно хотите удалить этап выполнения задачи личного состава без возможности восстановления?',
        acceptIcon: 'pi-trash',
        reject: 'Отмена',
        accept: 'Да',
      },
    }).onClose.pipe(takeWhile(res => res)).subscribe(() => {
        this.loader.startLoading(this.newEventService.deleteCompleting(uuid)).subscribe(value => this.getCompletingTable());
      },
  ...
  private setJumpState(): void {
    this.newEventService.getJumpsState().subscribe(value => {
  ...
  private getAllAerodrome(): void {
    this.loader.startLoading(this.newEventService.getAerodrome()).subscribe(value => this.airportDeparture = value.data);
  }

  private getAllAircraft(): void {
    this.loader.startLoading(this.newEventService.getAircraftDirectory()).subscribe(value => this.allAirplane = value.data);
  }
  ...
  private getStaffInfo(): void {
    this.loader.startLoading(this.newEventService.getStaffTaskFullInfo(this.newTaskUUID)).subscribe(value => this.setTaskFullInfo(value));
  }
  ...
  public saveDocuments(): void {
    ...
    this.loader.startLoading(this.newEventService.putTaskDocuments(this.newTaskUUID, docParams)).subscribe(() => this.getTaskDocs());
  }

  private getTaskDocs(): void {
    this.loader.startLoading(this.newEventService.getTaskDocuments(this.newTaskUUID)).subscribe(value => this.documents = value?.document);
  }
  ...
  private setTaskFullInfo(value): void {
    ...
    const fullDateStart = new Date(value.date_start_plan);
    const fullDateEnd = new Date(value.date_end_plan);
    const onlyTimeStart = new Date(dateNow.getFullYear(), dateNow.getMonth(), dateNow.getDate(), fullDateStart.getHours(), fullDateStart.getMinutes());
    const onlyTimeEnd = new Date(dateNow.getFullYear(), dateNow.getMonth(), dateNow.getDate(), fullDateEnd.getHours(), fullDateEnd.getMinutes());
    ...
    this.form.controls.time_start_plan?.setValue(onlyTimeStart);
    this.form.controls.time_end_plan?.setValue(onlyTimeEnd);
