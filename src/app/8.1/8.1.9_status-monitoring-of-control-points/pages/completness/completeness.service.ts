...
export class CompletenessService {
  ...
  getCompletenessHierarchy(uuid: string, date?: Date): Observable<any> {
    ...
    return this.httpClient.get<any>(`api/oshs_staffing/${uuid}`, { params: httpParams });
  }

  createNewShdk(shdkId: number, data: any): Observable<any> {
    return this.httpClient.post<any>(`api/shdk/add_new_shdk/${shdkId}`, data);
  }
